					...
					<Link prefetch href={`/videos/${data.id}`} className="flex-1 min-w-0">
						...
						{size === "default" && (
							<p className="text-xs text-muted-foreground mt-1">
								{compactViews} 观看 • {compactLikes} 喜欢
							</p>
						)}
						{size === "default" && (
							<>
								<div className="flex items-center gap-2 my-3">
									<UserAvatar
										size="sm"
										imageUrl={data.user.imageUrl || AVATAR_FALLBACK}
										name={data.user.name}
									/>
									<UserNameSpan size="sm" name={data.user.name} />
								</div>
								<Tooltip>
						...
						{size === "compact" && <UserNameSpan size="sm" name={data.user.name} />}
						{size === "compact" && (
							<p className="text-xs text-muted-foreground mt-1">
								{compactViews} 观看 • {compactLikes} 喜欢
							</p>
